...
| Ebene | Befehl | Umfang |
| --- | --- | --- |
| Unit / Service (Client) | `npm run test:unit` | 1617 Tests, jsdom, Firestore gemockt |
...
Der globale Wert (aktuell ~83 % Statements, ~74 % Branches) ist bewusst niedriger: Er umfasst
...
- `useImageOptimization` hatte das Optionsobjekt als Effekt-Abhängigkeit und
  lief bei inline übergebenen Optionen in eine Render-Endlosschleife.
- `useCacheOptimization` behandelte `staleTime: 0` wie den Standardwert von
  fünf Minuten (`||` statt `??`) und lud nie neu.
